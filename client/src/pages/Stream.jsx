...
export default function Stream({ streaming, mediaURL }) {
  ...
  useEffect(() => {
    if (audioIO.current != null) {
      ...
      if (trackNames.current[track] == "main") {
        ...
        trackNames.current[track] == "cellos" ||
        trackNames.current[track] == "flutes" ||
        trackNames.current[track] == "violins"
      ) {
      ...
      if (audioIO.current != null) {
        ...
        if (rotation != 0) {
          ...
          const response = await fetch(
            `/stream/${streaming._id}/hrtfs/${rotation}`
          );
  ...
  const onPlay = async () => {
    ...
    if (audioIO.current != null) return;
    ...
    numChannels.current = tracks.map((track) =>
      parseInt(track.audioChannelConfiguration)
    );
    ...
    audioIO.current.addAudioChain(
      new SSSAudioChain(
        audioIO.current.getAudioCtx(),
        maxNumChannels,
        defaultGain,
        hrtfs
      )
    ...
    audioIO.current.addAudioChain(
      new MOAudioChain(audioIO.current.getAudioCtx(), 2, defaultGain)
    );
    audioIO.current.addAudioChain(
      new AmbiAudioChain(
        audioIO.current.getAudioCtx(),
    ...
    console.log(
      `Selected track name = ${trackNames.current[mainTrackIndex.current]}`
    );
    if (trackNames.current[mainTrackIndex.current] == "main") {
      ...
      trackNames.current[mainTrackIndex.current] == "cellos" ||
      trackNames.current[mainTrackIndex.current] == "flutes" ||
      trackNames.current[mainTrackIndex.current] == "violins"
    ) {
      audioIO.current.switchAudioChain(1);
      //audioChain.current = new AudioChain(audioRef.current,
      //     maxNumChannels, defaultGain, hrtfs);
  ...
  return (
    <div>
      <audio ref={audioRef} onPlay={onPlay} controls></audio>
